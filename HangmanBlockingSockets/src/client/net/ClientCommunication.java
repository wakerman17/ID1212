...
public class ClientCommunication{
	...
	public void addClient (InetSocketAddress serverAddress, ConsoleOutput consoleOutput) throws IOException {
		addClientPrivate (serverAddress, consoleOutput);
	}
	
	private void addClientPrivate (InetSocketAddress serverAddress, ConsoleOutput consoleOutput) throws IOException {
		socket = new Socket();
            ...
            try {
                while (true) {
                	...
                	if (msg.getType() == MsgType.GAMESTATE) {
                		outputHandler.handleGameState(msg.getGameState());
                	}
                }
            } catch (Throwable connectionFailure) {
                if (connected) {
                    outputHandler.handleErrorMessage();
                }
            }
        }
    }
